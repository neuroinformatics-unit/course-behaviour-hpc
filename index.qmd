...
```
atyson@hpc-gw2:~$ sinfo
PARTITION AVAIL  TIMELIMIT  NODES  STATE NODELIST
cpu*          up 10-00:00:0     10 drain~ enc1-node[2,4,6,8,10-12],enc2-node[4,7-8]
cpu*          up 10-00:00:0      3  idle~ enc1-node[3,7],enc2-node10
cpu*          up 10-00:00:0      5  down~ enc1-node[14,16],enc2-node[3,6,12]
cpu*          up 10-00:00:0      3  drain enc3-node[6-8]
cpu*          up 10-00:00:0     10    mix enc1-node[1,5,9,13],enc2-node[1-2,5],enc3-node[2,4-5]
cpu*          up 10-00:00:0      3  alloc enc2-node[9,11,13]
cpu*          up 10-00:00:0      1   idle enc3-node1
gpu           up 10-00:00:0      1 drain* gpu-350-02
gpu           up 10-00:00:0      1  drain gpu-350-03
gpu           up 10-00:00:0      5    mix gpu-350-01,gpu-380-[10,12-13],gpu-sr670-20
gpu           up 10-00:00:0     12   idle gpu-350-[04-05],gpu-380-[11,14-18],gpu-sr670-[21-23],gpu-sr675-31
a100          up 10-00:00:0      1    mix gpu-sr670-20
a100          up 10-00:00:0      3   idle gpu-sr670-[21-23]
gpu_lowp      up 10-00:00:0      5   idle gpu-sr675-[31-34],gpu-xd670-30
gpu_branco    up 30-00:00:0      3   idle gpu-sr675-[32-34]
gatsby_ws     up 10-00:00:0      1  down* u236b
gatsby_ws     up 10-00:00:0      4   idle adrian,u237h,u240c,u434a
...
## Interactive job
Start an interactive job (`bash -i`) in the cpu partition (`-p cpu`) in pseudoterminal mode (`--pty`) with one CPU core (`-n 1`).
```bash
srun -p cpu -n 1 --pty bash -i
```
. . .
...
## SLEAP workflow {.smaller}

![Source: [animals-in-motion.neuroinformatics.dev](https://animals-in-motion.neuroinformatics.dev/latest/03-sleap-tutorial.html)](img/diagrams/pose-estimation.png){fig-align=center height=240px}


:::: {.fragment}

::: {.callout-warning}
This workshop uses **SLEAP version 1.3.4** (TensorFlow backend), with documentation available at [legacy.sleap.ai](https://legacy.sleap.ai/).

HPC modules for [SLEAP ≥ 1.5.0](https://docs.sleap.ai/) (PyTorch backend) will be added in future.
:::

::::

## Sample data {.smaller}
`/ceph/scratch/neuroinformatics-dropoff/SLEAP_HPC_test_data/CalMS21`

- Contains an example video from the [Caltech Mouse Social Interactions (CalMS21) Dataset](https://sites.google.com/view/computational-behavior/our-datasets/calms21-dataset) (Sun et al. 2021).
- SLEAP project, made by **Chang Huan Lo**, with labeled frames, trained models and prediction results.

::: {.callout-tip}
## More details

See [animals-in-motion.neuroinformatics.dev/latest/prerequisites](https://animals-in-motion.neuroinformatics.dev/latest/prerequisites.html#sec-data) for more information about the data.
:::
...
::: aside
See also: [animals-in-motion.neuroinformatics.dev](https://animals-in-motion.neuroinformatics.dev/latest/03-sleap-tutorial.html#configure-and-train-models) and [SLEAP's guide for remote training](http://legacy.sleap.ai/guides/remote.html)
:::
...
cd /ceph/scratch/neuroinformatics-dropoff/SLEAP_HPC_test_data/CalMS21
cp -r mouse044_task1_annotator1.training_job /ceph/scratch/$USER/
cd /ceph/scratch/$USER/mouse044_task1_annotator1.training_job
ls -1
```
...
mouse044_task1_annotator1.train.pkg.pkg.slp  # Copy of labeled frames
multi_class_topdown.json                     # Model configuration
...
## What's in the SLEAP scripts? {.smaller}
...
```{.bash code-line-numbers=false}
#!/bin/bash
sleap-train centroid.json mouse044_task1_annotator1.train.pkg.pkg.slp
sleap-train multi_class_topdown.json mouse044_task1_annotator1.train.pkg.pkg.slp
```
...
```{.bash filename="sleap_train_slurm.sh" code-line-numbers="1-13|15-16|18-19|21-26|28-29"}
#!/bin/bash
...
```{.bash code-line-numbers="1-3|4-5"}
cd /ceph/scratch/neuroinformatics-dropoff/SLEAP_HPC_test_data/CalMS21/better_model
cp -R 250806_174722.centroid.n=679 /ceph/scratch/$USER/mouse044_task1_annotator1.training_job/models/
cp -R 250807_162146.multi_class_topdown.n=679 /ceph/scratch/$USER/mouse044_task1_annotator1.training_job/models/
cd /ceph/scratch/$USER/mouse044_task1_annotator1.training_job/models
ls -1
```
...
```{.bash code-line-numbers="true"}
cd 250806_174722.centroid.n=679
ls -1
```
...
::: aside
See also the SLEAP [model evaluation notebook](http://legacy.sleap.ai/notebooks/Model_evaluation.html){preview-link="true"}
:::


## SLEAP workflow

![](img/diagrams/pose-estimation.png){fig-align=center height=240px}

::: aside
Source: [animals-in-motion.neuroinformatics.dev](https://animals-in-motion.neuroinformatics.dev/latest/03-sleap-tutorial.html)
:::


## Batch script for inference {.smaller}

```{.bash filename="sleap_infer_slurm.sh" code-line-numbers="1-19|21-25|27-31|33-40"}
#!/bin/bash
...
```{.bash filename="sleap_infer_array_slurm.sh" code-line-numbers="14|27-29|37-44"}
#!/bin/bash
...
## Further reading
* [SWC/GCNU Scientific Computing wiki](https://liveuclac.sharepoint.com/sites/SSC)
